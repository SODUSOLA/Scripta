...
const DAILY_GENERATION_LIMIT = 10; // Daily limit per user
...
export async function generatePost({ userId, topic, tone, platform, regenerate = false }) {
    if (!topic) {
        throw new Error("Topic or prompt required");
    }
    ...
    // Check cache if regenerate = false
    if (!regenerate) {
    ...
    // Prepare the prompt
    const prompt = `
    Generate a bit lenghty, engaging social media post.
    Platform: ${platform || "Generic"}.
    Tone: ${tone || "Neutral"}.
    Topic: ${topic}.
    Keep it concise (at most 300 characters if X/Twitter, 3000 if LinkedIn, 100 if Facebook, 2000 if Instagram).
    Include natural phrasing suitable for ${platform || "general"} readers.
    `;
    ...
    // Approximate token count (Gemini doesn’t expose usage)
    const tokensUsed = Math.round(prompt.split(" ").length * 1.3 + generatedText.split(" ").length);
    const costUSD = (tokensUsed / 1000) * 0.0005; // Example rate
    ...
    // --- Save to cache (replace old entry)
