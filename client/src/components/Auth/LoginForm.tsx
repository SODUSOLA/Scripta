...
export default function LoginForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // Save token and email, redirect to dashboard
      localStorage.setItem("token", data.token);
      localStorage.setItem("userEmail", data.email || username);
      setError(null);

      // Dispatch custom event to update UserContext
  ...
  function handleGoogleAuth() {
    ...
    alert("Start Google OAuth flow (implement server/client flow).");
  }
      ...
      <div className="relative z-10 w-full max-w-[1300px] mx-auto flex flex-col lg:flex-row items-center justify-between min-h-screen px-4 lg:px-10 py-8">
        ...
        <div className="w-full max-w-md mx-auto lg:ml-auto lg:mr-0 flex flex-col items-center justify-center">
          <div className="bg-white/95 rounded-xl shadow-xl px-8 py-10 w-full flex flex-col items-center border border-gray-200 backdrop-blur-md transition-all duration-200">
            ...
            >
              {error && (
                <div className="text-red-600 text-sm font-medium py-2 px-4 bg-red-50 border border-red-200 rounded-lg mb-2 text-center animate-fade-in">
                  {error}
                </div>
              )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Link
                    to="/reset"
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-blue-600 font-semibold px-2 py-0.5 hover:underline focus:outline-none focus:ring-2 focus:ring-blue-400 rounded"
                  >
                    Forgot?
                  </Link>
                </div>
            ...
            <div className="mt-7 w-full">
              <button
                ...
                aria-label="Authorize login with Google"
              >
